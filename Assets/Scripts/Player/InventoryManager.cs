...
public class InventoryManager : MonoBehaviour
{
    ...
    public List<UnityEngine.UI.Image> weaponUISlots = new List<UnityEngine.UI.Image>(6);
    public List<PassiveItem> passiveItemSlots = new List<PassiveItem>(6);
    public int[] passiveItemLevels = new int[6];
    public List<UnityEngine.UI.Image> passiveItemUISlots = new List<UnityEngine.UI.Image>(6);
    ...
    public void AddWeapon(int slotIndex, WeaponController weapon) //Adds weapon to a slot
    {
    ...
    public void AddPassiveItem(int slotIndex, PassiveItem passiveItem) //Adds a passive item to a slot
    {
        passiveItemSlots[slotIndex] = passiveItem;
        passiveItemLevels[slotIndex] = passiveItem.passiveItemData.Level;
        passiveItemUISlots[slotIndex].enabled = true; //Enable image
    ...
    public void LevelUpWeapon(int slotIndex)
    {
        if(weaponSlots.Count > slotIndex)
        {
            ...
            if (!weapon.weaponData.NextLevelPrefab) //Checks if there is a next level available for weapons
            {
                Debug.LogError("NO NEXT LEVEL FOR " + weapon.name);
                return;
            }
            GameObject upgradedWeapon = Instantiate(weapon.weaponData.NextLevelPrefab, transform.position, UnityEngine.Quaternion.identity);
            upgradedWeapon.transform.SetParent(transform);
            AddWeapon(slotIndex, upgradedWeapon.GetComponent<WeaponController>());
            Destroy(weapon.gameObject);
            weaponLevels[slotIndex] = upgradedWeapon.GetComponent<WeaponController>().weaponData.Level; //Makes sure the current weapon level is correct
    ...
    public void LevelUpPassiveItem(int slotIndex)
    {
        if(passiveItemSlots.Count > slotIndex)
        {
            ...
            if (!passiveItem.passiveItemData.NextLevelPrefab) //Checks if there is a next level available for passive items
            {
                Debug.LogError("NO NEXT LEVEL FOR " + passiveItem.name);
                return;
            }
            GameObject upgradedPassiveItem = Instantiate(passiveItem.passiveItemData.NextLevelPrefab, transform.position, UnityEngine.Quaternion.identity);
